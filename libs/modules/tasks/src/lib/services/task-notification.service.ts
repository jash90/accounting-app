...
export class TaskNotificationService {
  ...
  async notifyTaskCreated(task: Task, performedBy: User): Promise<void> {
    ...
    if (!smtpConfig) {
      this.logger.warn('No active email configuration for company. Skipping task create notifications.', {
  ...
  async notifyTaskUpdated(
    ...
    const clientIdChanged = oldClientId !== currentClientId;
    ...
    if (!smtpConfig) {
      this.logger.warn('No active email configuration for company. Skipping task update notifications.', {
        companyId: task.companyId,
        taskId: task.id,
      });
      return;
  ...
  async notifyTaskDeleted(task: Task, performedBy: User): Promise<void> {
    ...
    if (!smtpConfig) {
      this.logger.warn('No active email configuration for company. Skipping task delete notifications.', {
        companyId: task.companyId,
        taskId: task.id,
      });
      return;
