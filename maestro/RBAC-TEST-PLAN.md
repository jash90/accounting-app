...
| --- | ----------------------- | ------- | ------------------------------------------------- |
| 1   | `/admin`                | TODO    | `auth/employee-cannot-access-admin.yaml`          |
| 2   | `/admin/users`          | TODO    | (consolidate into above as multi-step)            |
| 3   | `/admin/companies`      | TODO    | (consolidate)                                     |
| 4   | `/admin/email-config`   | TODO    | (consolidate)                                     |
| 5   | `/company`              | TODO    | `auth/employee-cannot-access-company.yaml`        |
| 6   | `/company/profile`      | ✅ DONE | `auth/employee-rbac-denied.yaml`                  |
| 7   | `/company/employees`    | TODO    | (consolidate into employee-cannot-access-company) |
| 8   | `/company/modules`      | TODO    | (consolidate)                                     |
| 9   | `/company/email-config` | TODO    | (consolidate)                                     |

### COMPANY_OWNER forbidden routes

| #   | Route                 | Status | Proposed file                         |
| --- | --------------------- | ------ | ------------------------------------- |
| 10  | `/admin`              | TODO   | `auth/owner-cannot-access-admin.yaml` |
| 11  | `/admin/users`        | TODO   | (consolidate)                         |
| 12  | `/admin/companies`    | TODO   | (consolidate)                         |
| 13  | `/admin/email-config` | TODO   | (consolidate)                         |

### ADMIN forbidden routes

ADMIN has no `companyId` — behavior on company/employee routes needs verification first.

| #   | Route              | Expected (verify in Chrome first)        | Proposed file                           |
| --- | ------------------ | ---------------------------------------- | --------------------------------------- |
| 14  | `/company`         | unknown — likely redirect or empty state | `auth/admin-cannot-access-company.yaml` |
| 15  | `/company/profile` | unknown                                  | (same)                                  |
| 16  | `/modules`         | unknown                                  | (same)                                  |
...
**Status:** EMPLOYEE flow already asserts `assertNotVisible: "Pracownicy"`. Two more roles + extra labels per role need to be added.

**Effort:** ~30 min. No new files. Edit `auth/login.yaml`, `auth/login-employee.yaml`, `auth/login-admin.yaml` only.
...
| --- | -------------------- | ----------------------- | ------------------------------------ |
| 17  | no auth cookie/token | open `/company/profile` | render `Witaj ponownie` (login form) |
| 18  | no auth              | open `/admin`           | login form                           |
| 19  | no auth              | open `/modules`         | login form                           |
| 20  | no auth              | open `/notifications`   | login form                           |
...
1. **P2 — sidebar visibility** (no new files, no risk, ~30 min) — start here, easiest win
2. **P1 — denied-access matrix** (3 new flow files, ~2-3 h) — biggest pure-RBAC coverage gain
3. **P3 — logged-out redirects** (1 new flow file, ~30 min)
...
Total Maestro additions in P1+P2+P3 = ~4 new flow files + 3 edits = solid baseline. P4-P6 are tracked here as the longer-term plan.
