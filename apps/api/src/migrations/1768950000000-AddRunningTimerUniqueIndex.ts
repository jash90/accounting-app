...
export class AddRunningTimerUniqueIndex1768950000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (duplicates && duplicates.length > 0) {
      console.log(
        `[AddRunningTimerUniqueIndex] Found ${duplicates.length} users with duplicate running timers. Auto-fixing...`
      );

      // Auto-fix: For each user with duplicates, keep only the most recently started timer running
      for (const dup of duplicates) {
        console.log(
          `[AddRunningTimerUniqueIndex] Fixing duplicates for userId=${dup.userId}, companyId=${dup.companyId} (${dup.count} running timers)`
        );
        ...
        for (const entry of entriesToStop) {
          ...
          `,
            [
              ...
              JSON.stringify([
                {
                  field: '_migrationNote',
                  newValue:
                    'Auto-stopped: duplicate running timer detected during migration AddRunningTimerUniqueIndex',
                },
          ...
          console.log(
            `[AddRunningTimerUniqueIndex] Logged auto-stop for entry ${entry.id} (started: ${entry.startTime})`
          );
        }
      ...
      if (remainingDuplicates[0]?.count > 0) {
        throw new Error(
          `Failed to fix duplicate running timers. ${remainingDuplicates[0].count} duplicates remain.`
        );
      }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."IDX_time_entries_one_running_per_user"`
    );
